...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		//转换bean名称，主要是两类，一类是应用于获取factory-bean本身的语法&&&，另一类是解析别名
		...
		if (sharedInstance != null && args == null) {
			...
			if (!typeCheckOnly) {
				//疑问，此处有什么用？表示bean已经创建了，后面在合并Beandefinition的时候会进行缓存mergedbean
				//参考isBeanEligibleForMetadataCaching方法
				markBeanAsCreated(beanName);
			}

			try {
				//此方法用于转换、合并beandefinition，比如合并ChildBeanDefinition和其父Bean，并最终
				//转换为RootBeanDefinition
				final RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
				//检测beandefinition，两方面，一、不能指定beandefinition为抽象类。二、如果指定了args则mbd类型必须为prototype
				checkMergedBeanDefinition(mbd, beanName, args);

				// Guarantee initialization of beans that the current bean depends on.
				//bean定义带有dependon属性，如果A-->B（A依赖B），则初始化A的时候需要先初始B，保证了初始化的顺序。
		...
		synchronized (this.mergedBeanDefinitions) {
			...
			if (mbd == null) {
				if (bd.getParentName() == null) {
					...
					//合并父bean，合并的时候需要考虑延伸，如果父bean也指定了parent则需要一直解析下去
					BeanDefinition pbd;
					try {
					...
					//此处为覆盖逻辑，也即子类属性覆盖父类，且从父类继承属性
					mbd.overrideFrom(bd);
				}

				// Set default singleton scope, if not configured before.
				//此处显示出spring中bean属性默认为singleton
				...
				if (containingBd != null && !containingBd.isSingleton() && mbd.isSingleton()) {
					//如果父类bean不是单例，但是该bean是单例，则修正scope为父bean的scope
